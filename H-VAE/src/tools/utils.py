...
def read_data():
    omics_df = {}
    omics = ['ge', 'methyl', 'pe']
    paths = ['~/tese/UCSC_TCGA_Data_Clean/Clean/ge_clean.tsv', '~/tese/UCSC_TCGA_Data_Clean/Clean/dna_clean.tsv', '~/tese/UCSC_TCGA_Data_Clean/Clean/pe_clean.tsv']

    for i, omic in enumerate(omics):
        omics_df[omic] = pd.read_csv(paths[i], sep='\t', index_col=0, header=0).T
        print(f'{omic} shape: {omics_df[omic].shape}')
    
    clinical_df = pd.read_csv('~/tese/UCSC_TCGA_Data_Clean/Clean/clinical.tsv', sep='\t', index_col=0, header=0)
    lt_samples = list(clinical_df.index)
    return omics_df, clinical_df, lt_samples
...
def save_stratified_splits(clinical_df, cohort, n):
    lt_samples = list(clinical_df.index)
    labels = clinical_df['Tumor'].values

    skf = StratifiedKFold(n_splits=n, shuffle=True, random_state=42)

    for i, (train_index, test_index) in enumerate(skf.split(lt_samples, labels)):
        train_index, val_index = train_test_split(train_index, test_size=0.25, stratify=labels[train_index])

        samples_train = [lt_samples[j] for j in train_index]
        samples_val = [lt_samples[j] for j in val_index]
        samples_test = [lt_samples[j] for j in test_index]
...
def save_plot_score(filename, z, y, title, show, method='tsne'):
    if method == 'tsne':
        reducer = TSNE(n_components=2, verbose=0, perplexity=40, n_iter=300)
    elif method == 'umap':
