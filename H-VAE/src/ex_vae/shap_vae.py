...
def processPhenotypeDataForSamples(clinical_df, sample_id):
...
def sample_background_and_tumor(expr_df, conditionaltumour, n_samples):
    background_expr = expr_df.loc[~conditionaltumour, :]
    tumor_expr = expr_df.loc[conditionaltumour, :]
    
    max_background = len(background_expr)
    max_tumor = len(tumor_expr)
    n_samples = min(n_samples, max_background, max_tumor)
    
    background_sample = background_expr.sample(n=n_samples, axis=0, random_state=42)
    tumor_sample = tumor_expr.sample(n=n_samples, axis=0, random_state=42)
    return background_sample, tumor_sample
